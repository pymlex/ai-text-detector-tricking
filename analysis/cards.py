...
def _metrics_table(snapshot: AnalysisSnapshot) -> str:
    ...
    return "\n".join(
        [
            ...
            f"{report.validation.roc_auc:.4f} | {report.validation.f1:.4f} |",
            ...
            f"{report.test.roc_auc:.4f} | {report.test.f1:.4f} |",
        ]
    )
...
Probe size: {probe.n_samples} paraphrase pairs from the train split. Mean $|z_1-z_2|$: {probe.mean_abs_logit_diff:.4f}. Median: {probe.median_abs_logit_diff:.4f}. 75th percentile: {probe.p75_abs_logit_diff:.4f}. Maximum gap: {probe.max_abs_logit_diff:.4f}.
...
Preference pairs for DPO fine-tuning of `{MODEL_ID}` against the Oculus multilingual AI text detector on Spanish academic abstracts.
...
Logit margin threshold: $|z_1-z_2| \\ge {pref.logit_margin_threshold:g}$.
